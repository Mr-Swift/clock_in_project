...
public abstract class RSAUtils {
    ...
    /**
     * 使用给定的私钥解密由JS加密（使用此类提供的公钥加密）的字符串。
     * <p />
     * 若私钥为 {@code null}，或者 {@code encrypttext} 为 {@code null}或空字符串则返回 {@code null}。
     * 私钥不匹配时，返回 {@code null}。
     * 
...
//    
    /** 私钥转String。*/
    public static String getPrivateKeyString (RSAPrivateKey privateKey) throws IOException{
...
//      System.out.println(new String(Hex.encodeHex(publicKey.getPublicExponent().toByteArray())));
        ...
        String prkey=getPrivateKeyString(privateKey);
        System.out.println("prkey: "+prkey+"     "+ privateKey.equals(getPrivateKey(prkey))+"     "+privateKey);
