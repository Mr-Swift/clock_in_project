...
public class Util {
    ...
    public static String formatCardNum(String cardNum) {
        if (Util.n(cardNum) || cardNum.length() != 18) {
            log.info("invalid cardNumber, cardNumber = " + cardNum);
            return cardNum;
        }
...
//        return idCardNumber.replaceAll("(\\w{10})\\w*(\\w{2})", "$1******$2");//这个模式的例子 3211811990******10
    ...
    public static void main(String[] args) {
        System.out.println(DateUtil.date(LocalDateTime.of(LocalDate.of(Integer.parseInt("2018"), LocalDate.now().getMonth(), 1), LocalTime.MIN)));
        System.out.println(DateUtil.date(LocalDateTime.of(LocalDate.of(Integer.parseInt("2018"), LocalDate.now().getMonth(), LocalDate.now().getDayOfMonth()), LocalTime.MAX)));
