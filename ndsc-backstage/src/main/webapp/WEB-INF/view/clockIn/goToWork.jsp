...
<head>
    ...
    <script type="text/javascript">
        ...
        function add() {
            $.ajax({
                type: "post",
                url: '<%=request.getContextPath()%>/goToWork/add.do',
                traditional: true,
                dataType:"json",
        ...
        function toExe() {
            $("#shadow").table2excel({
                ...
                filename: "上班打卡" + new Date().getFullYear() + "年" + new Date().getMonth() + 1 + "月" + new Date().getDate() + "日" + ".xls",
                exclude_img: true,
                exclude_links: true,
                exclude_inputs: true
            });
    ...
    <script type="text/javascript">
        function goToPage() {
            ...
            if (page != null && page != ''){
                ...
                window.location.href = "<%=request.getContextPath()%>/goToWork/toGoToWork.do?page=" + page+"&year"+year+"&month"+month+"&day"+day+"&remark"+remark;
            } else {
                alert("跳转失败，请重新输入页数");
            }
        }
        ...
        function toPreviousPage(page){
            if (Number(page) == 0) {
                ...
                location.href="<%=request.getContextPath()%>/goToWork/toGoToWork.do?page=${page-1}&year=${result.params.get('year')}&month=${result.params.get('month')}&day=${result.params.get('day')}&remark=${result.params.get('remark')}"
            }
        }
        ...
        function toNextPage(page) {
            ...
            if (Number(page) > Number(sumCount)) {
                ...
                window.location.href = "<%=request.getContextPath()%>/goToWork/toGoToWork.do?page=${page+1}&year=${result.params.get('year')}&month=${result.params.get('month')}&day=${result.params.get('day')}&remark=${result.params.get('remark')}"
            }
        }
    </script>
</head>
...
<div class="pd-zy20">
    ...
    <div class="cx-bt">
        ...
        <div class="btn-bt">
            <ul>
                <li>
                    <a href="javascript:;" onclick="add();">上班打卡</a>
        ...
        <div class="search1">
            <ul>
                <form action="<%=request.getContextPath()%>/goToWork/toGoToWork.do" id="select">
...
<%--                        <c:if test="${not empty result.params.get('year')}">--%>
<%--                               value="${result.params.get('year')}"--%>
<%--                        </c:if>--%>
                        <%--                                   value="${year}-${month}"--%>
...
<%--                        <c:if test="${not empty result.params.get('month')}">--%>
<%--                               value="${result.params.get('month')}"--%>
<%--                        </c:if>--%>
...
<%--                        <c:if test="${not empty result.params.get('day')}">--%>
<%--                               value="${result.params.get('day')}"--%>
<%--                        </c:if>--%>
                    ...
                    <li>打卡时间：</li>
                    <li>
                        <select id="remark" name="remark" style="width: 100px"
                        >
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="fy">
        <tr>
            <td height="40" align="right" >
                ...
                <a style="width: 30px" href="<%=request.getContextPath()%>/goToWork/toGoToWork.do?page=1">首页</a>&nbsp;|&nbsp;
                ...
                <a style="width: 30px" href="<%=request.getContextPath()%>/goToWork/toGoToWork.do?page=${totalPage}">尾页</a> 跳转到</td>
