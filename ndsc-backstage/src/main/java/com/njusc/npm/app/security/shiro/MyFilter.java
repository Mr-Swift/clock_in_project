...
public class MyFilter extends AuthenticatingFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
    	...
    	request1.getSession().setAttribute("returnUrl", request1.getRequestURI());
        ...
        if (isLoginRequest(request, response)) {
    		...
    		if ("XMLHttpRequest".equalsIgnoreCase(request1.getHeader("x-requested-with"))){
    			response1.setHeader("consoleSessionStatus", "timeout");//
    		}
            PrintWriter out = response1.getWriter();  
            StringBuilder builder = new StringBuilder();  
            builder.append("<script type=\"text/javascript\" charset=\"UTF-8\">");  
//            builder.append("alert(\"页面过期，请重新登录11\");");  
            builder.append("window.top.location.href=\"");  
            //builder.append(Constants.basePath);  
            builder.append(request1.getContextPath()+"/login/toLogin.do\";</script>");
